...
class JWTTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $msg = JWT::getInstance()->encode('abc', 'my_key');
        $this->assertEquals(JWT::getInstance()->decode($msg, 'my_key', array('HS256')), 'abc');
    }
    ...
    public function testDecodeFromPython()
    {
        ...
        $this->assertEquals(
            JWT::getInstance()->decode($msg, 'my_key', array('HS256')),
    ...
    {
        $encoded = JWT::getInstance()->encode('f?', 'a');
        $this->assertEquals('f?', JWT::getInstance()->decode($encoded, 'a', array('HS256')));
    }

    public function testMalformedUtf8StringsFail()
    {
        $this->setExpectedException('DomainException');
        JWT::getInstance()->encode(pack('c', 128), 'a');
    }

    public function testMalformedJsonThrowsException()
    {
        $this->setExpectedException('DomainException');
        JWT::getInstance()->jsonDecode('this is not valid JSON string');
    }
    ...
    public function testExpiredToken()
    {
        ...
        $encoded = JWT::getInstance()->encode($payload, 'my_key');
        JWT::getInstance()->decode($encoded, 'my_key', array('HS256'));
    }
    ...
    public function testValidToken()
    {
        $payload = array(
            "message" => "abc",
            "exp" => time() + JWT::getInstance()->leeway + 20); // time in the future
        $encoded = JWT::getInstance()->encode($payload, 'my_key');
        $decoded = JWT::getInstance()->decode($encoded, 'my_key', array('HS256'));
        $this->assertEquals($decoded->message, 'abc');
    }

    public function testValidTokenWithLeeway()
    {
        JWT::getInstance()->leeway = 60;
        $payload = array(
            "message" => "abc",
            "exp" => time() - 20); // time in the past
        $encoded = JWT::getInstance()->encode($payload, 'my_key');
    ...
    public function testValidTokenWithList()
    {
        ...
        $encoded = JWT::getInstance()->encode($payload, 'my_key');
        $decoded = JWT::getInstance()->decode($encoded, 'my_key', array('HS256', 'HS512'));
        $this->assertEquals($decoded->message, 'abc');
    }
    ...
    public function testInvalidToken()
    {
        ...
        $decoded = JWT::getInstance()->decode($encoded, 'my_key2', array('HS256'));
    }
    ...
    public function testNullKeyFails()
    {
        ...
        $decoded = JWT::getInstance()->decode($encoded, null, array('HS256'));
    }
    ...
    public function testEmptyKeyFails()
    {
        ...
        $encoded = JWT::getInstance()->encode($payload, 'my_key');
        $this->setExpectedException('InvalidArgumentException');
        $decoded = JWT::getInstance()->decode($encoded, '', array('HS256'));
    }
    ...
    public function testRSEncodeDecode()
    {
        ...
        $msg = JWT::getInstance()->encode('abc', $privKey, 'RS256');
        $pubKey = openssl_pkey_get_details($privKey);
        $pubKey = $pubKey['key'];
        $decoded = JWT::getInstance()->decode($msg, $pubKey, array('RS256'));
    ...
    public function testKIDChooser()
    {
        ...
        $msg = JWT::getInstance()->encode('abc', $keys['1'], 'HS256', '1');
        $decoded = JWT::getInstance()->decode($msg, $keys, array('HS256'));
        $this->assertEquals($decoded, 'abc');
    }
    ...
    public function testNoneAlgorithm()
    {
        ...
        JWT::getInstance()->decode($msg, 'my_key', array('none'));
    }
    ...
    public function testIncorrectAlgorithm()
    {
        $msg = JWT::getInstance()->encode('abc', 'my_key');
        $this->setExpectedException('UnexpectedValueException');
        JWT::getInstance()->decode($msg, 'my_key', array('RS256'));
    }
    ...
    public function testMissingAlgorithm()
    {
        $msg = JWT::getInstance()->encode('abc', 'my_key');
        $this->setExpectedException('UnexpectedValueException');
        JWT::getInstance()->decode($msg, 'my_key');
    }

    public function testAdditionalHeaders()
    {
        $msg = JWT::getInstance()->encode('abc', 'my_key', 'HS256', null, array('cty' => 'test-eit;v=1'));
        $this->assertEquals(JWT::getInstance()->decode($msg, 'my_key', array('HS256')), 'abc');
    }
    ...
    {
        $this->setExpectedException('UnexpectedValueException');
        JWT::getInstance()->decode('brokenheader.brokenbody', 'my_key', array('HS256'));
    }
    ...
    public function testInvalidSignatureEncoding()
    {
        ...
        $this->setExpectedException('UnexpectedValueException');
        JWT::getInstance()->decode($msg, 'secret', array('HS256'));
    }
    ...
    public function testVerifyError()
    {
        ...
        $msg = JWT::getInstance()->encode('abc', $pkey, 'RS256');
        self::$opensslVerifyReturnValue = -1;
        JWT::getInstance()->decode($msg, $pkey, array('RS256'));
    }
}
